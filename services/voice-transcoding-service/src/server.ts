...
async function readBody(
  ...
  for await (const chunk of request) {
    ...
    size += buffer.length;
    if (size > limit) throw new Error("recording exceeds upload limit");
    chunks.push(buffer);
  }
...
export function createVoiceServer(
  ...
  const handleRequest = async (
    ...
    if (request.method === "OPTIONS") {
      response.writeHead(
        origin && config.allowedOrigins.has(origin) ? 204 : 403,
      );
      response.end();
      return;
    }
    ...
    try {
      ...
      if (input.length === 0) throw new Error("empty recording");
