create function public.media_storage_path_organization_id(
  p_bucket_id text,
  p_name text
) returns uuid
language plpgsql
...
create function public.media_storage_object_size_bytes(
  p_metadata jsonb
) returns bigint
language sql
immutable
...
create function public.media_storage_status(
  p_used_bytes bigint,
...
create function public.initialize_organization_media_storage() returns trigger
language plpgsql
security definer
set search_path to ''
as $$
...
create trigger initialize_organization_media_storage
after insert on public.organizations
for each row
execute function public.initialize_organization_media_storage();

create function public.apply_organization_media_storage_usage() returns trigger
language plpgsql
security definer
set search_path to ''
as $$
...
create trigger apply_organization_media_storage_usage
before insert or delete or update
on storage.objects
for each row
execute function public.apply_organization_media_storage_usage();

create function public.reconcile_organization_media_storage_internal(
...
create function public.get_organization_media_storage(
  p_organization_id uuid
) returns table (
...
create function public.get_platform_organization_media_storage(
  p_organization_id uuid
) returns table (
  organization_id uuid,
...
create function public.list_platform_media_storage_page(
...
create function public.update_platform_organization_media_storage_quota(
  p_organization_id uuid,
  p_quota_gb integer,
  p_request_id uuid
) returns table (
...
create function public.reconcile_platform_organization_media_storage(
  p_organization_id uuid,
  p_request_id uuid
) returns table (
  organization_id uuid,
...
create function public.reconcile_organization_media_storage_batch(
...
revoke execute on function public.get_organization_media_storage(uuid) from public, anon;
grant execute on function public.get_organization_media_storage(uuid) to authenticated;
