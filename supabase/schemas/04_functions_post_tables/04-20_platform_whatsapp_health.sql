create function public.platform_whatsapp_health_state(
  p_connection_status text,
  p_token_status text,
  p_webhook_status text,
  p_last_check_attempted_at timestamp with time zone,
  p_last_check_succeeded_at timestamp with time zone,
  p_token_expires_at timestamp with time zone,
  p_quality_rating text,
  p_last_webhook_error_at timestamp with time zone,
  p_failure_code text
) returns text
language sql
stable
...
create function public.list_platform_whatsapp_health_page(
  ...
  last_message_activity_at timestamp with time zone,
  ...
  webhook_error_count_24h bigint,
  failure_code text,
  failure_message text,
  template_status_summary jsonb,
  total_count bigint
)
language plpgsql
stable
...
create function public.get_platform_whatsapp_health(
  p_organization_id uuid,
...
create function public.record_platform_whatsapp_action(
  p_organization_id uuid,
  p_phone_number_id text,
  p_action_type text,
  p_request_id uuid,
...
create function public.get_platform_whatsapp_action_result(
  p_organization_id uuid,
  p_phone_number_id text,
  p_action_type text,
  p_request_id uuid
