...
drop function if exists "public"."list_platform_organization_agents_page"(p_organization_id uuid, p_page integer, p_page_size integer, p_search text);
  ...
  create table "public"."organization_agent_capacity" (
    ...
    "updated_by" uuid,
    "updated_by_scope" text not null default 'platform'::text,
...
CREATE UNIQUE INDEX organization_agent_capacity_pkey ON public.organization_agent_capacity USING btree (organization_id);

alter table "public"."organization_agent_capacity" add constraint "organization_agent_capacity_pkey" PRIMARY KEY using index "organization_agent_capacity_pkey";

alter table "public"."organization_agent_capacity" add constraint "organization_agent_capacity_limit_check" CHECK (((max_agent_seats IS NULL) OR (max_agent_seats > 0))) not valid;

alter table "public"."organization_agent_capacity" validate constraint "organization_agent_capacity_limit_check";

alter table "public"."organization_agent_capacity" add constraint "organization_agent_capacity_organization_fkey" FOREIGN KEY (organization_id) REFERENCES public.organizations(id) ON DELETE CASCADE not valid;

alter table "public"."organization_agent_capacity" validate constraint "organization_agent_capacity_organization_fkey";

alter table "public"."organization_agent_capacity" add constraint "organization_agent_capacity_scope_check" CHECK ((updated_by_scope = 'platform'::text)) not valid;

alter table "public"."organization_agent_capacity" validate constraint "organization_agent_capacity_scope_check";

alter table "public"."organization_agent_capacity" add constraint "organization_agent_capacity_updated_by_fkey" FOREIGN KEY (updated_by) REFERENCES auth.users(id) ON DELETE SET NULL not valid;

alter table "public"."organization_agent_capacity" validate constraint "organization_agent_capacity_updated_by_fkey";

alter table "public"."platform_admin_action_events" add constraint "platform_admin_action_events_action_check" CHECK ((action_type = ANY (ARRAY['routing_queue.create'::text, 'routing_queue.update'::text, 'organization_agent_capacity.update'::text, 'organization_agent.invite'::text, 'organization_agent.update'::text, 'organization_agent.remove'::text, 'organization_automation.update'::text, 'whatsapp.health_check'::text, 'whatsapp.profile_refresh'::text, 'whatsapp.template_sync'::text]))) not valid;

alter table "public"."platform_admin_action_events" validate constraint "platform_admin_action_events_action_check";

alter table "public"."platform_admin_action_events" add constraint "platform_admin_action_events_target_check" CHECK ((((target_type = 'routing_queue'::text) AND (action_type ~~ 'routing_queue.%'::text)) OR ((target_type = 'organization_agent_capacity'::text) AND (action_type = 'organization_agent_capacity.update'::text)) OR ((target_type = 'organization_agent'::text) AND (action_type ~~ 'organization_agent.%'::text)) OR ((target_type = 'organization_automation'::text) AND (action_type = 'organization_automation.update'::text)) OR ((target_type = 'whatsapp_account'::text) AND (action_type ~~ 'whatsapp.%'::text)))) not valid;

alter table "public"."platform_admin_action_events" validate constraint "platform_admin_action_events_target_check";

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.count_organization_agent_seats(p_organization_id uuid)
 RETURNS bigint
 LANGUAGE sql
...
CREATE OR REPLACE FUNCTION public.create_platform_organization_agent_invitation(p_organization_id uuid, p_name text, p_email text, p_request_id uuid)
...
CREATE OR REPLACE FUNCTION public.enforce_organization_agent_capacity()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION public.get_platform_organization_agent_audit_state(p_agent_id uuid)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION public.get_platform_organization_agent_capacity(p_organization_id uuid)
 RETURNS TABLE(organization_id uuid, max_agent_seats integer, used_agent_seats bigint, over_limit boolean, updated_at timestamp with time zone)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
AS $function$
...
CREATE OR REPLACE FUNCTION public.is_counted_agent_seat(p_ai boolean, p_extra jsonb)
 RETURNS boolean
 LANGUAGE sql
 IMMUTABLE
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION public.remove_platform_organization_agent(p_agent_id uuid, p_request_id uuid)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION public.update_platform_organization_agent(p_agent_id uuid, p_name text, p_routing_queue_ids uuid[] DEFAULT ARRAY[]::uuid[], p_request_id uuid DEFAULT NULL::uuid)
 RETURNS public.agents
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION public.update_platform_organization_agent_capacity(p_organization_id uuid, p_max_agent_seats integer, p_request_id uuid)
 RETURNS TABLE(organization_id uuid, max_agent_seats integer, used_agent_seats bigint, over_limit boolean, updated_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION public.list_platform_organization_agents_page(p_organization_id uuid, p_page integer DEFAULT 1, p_page_size integer DEFAULT 10, p_search text DEFAULT NULL::text)
 RETURNS TABLE(organization_id uuid, id uuid, user_id uuid, name text, email text, picture text, invitation_status text, queue_ids uuid[], queue_names text[], created_at timestamp with time zone, total_count bigint)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
AS $function$
...
grant references on table "public"."organization_agent_capacity" to "service_role";

grant trigger on table "public"."organization_agent_capacity" to "service_role";

grant truncate on table "public"."organization_agent_capacity" to "service_role";

CREATE TRIGGER enforce_organization_agent_capacity BEFORE INSERT OR UPDATE OF organization_id, ai, extra ON public.agents FOR EACH ROW EXECUTE FUNCTION public.enforce_organization_agent_capacity();

CREATE TRIGGER set_updated_at BEFORE UPDATE ON public.organization_agent_capacity FOR EACH ROW EXECUTE FUNCTION public.moddatetime('updated_at');
