alter table "public"."chatbot_flow_runs" add column "agent_id" uuid not null;

alter table "public"."chatbot_flow_runs" add column "lock_version" bigint not null default 0;

alter table "public"."chatbot_flow_runs" add constraint "chatbot_flow_runs_agent_fkey" FOREIGN KEY (organization_id, agent_id) REFERENCES public.agents(organization_id, id) ON DELETE RESTRICT not valid;

alter table "public"."chatbot_flow_runs" validate constraint "chatbot_flow_runs_agent_fkey";

alter table "public"."chatbot_flow_runs" add constraint "chatbot_flow_runs_lock_version_check" CHECK ((lock_version >= 0)) not valid;

alter table "public"."chatbot_flow_runs" validate constraint "chatbot_flow_runs_lock_version_check";

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.commit_chatbot_flow_execution(p_run_id uuid, p_expected_lock_version bigint, p_message_id uuid, p_current_node_id text, p_status text, p_waiting_for text, p_variables jsonb, p_error jsonb, p_outgoing_texts text[])
