drop function if exists "public"."commit_chatbot_flow_execution"(p_run_id uuid, p_expected_lock_version bigint, p_message_id uuid, p_current_node_id text, p_status text, p_waiting_for text, p_variables jsonb, p_error jsonb, p_outgoing_texts text[]);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.commit_chatbot_flow_execution(p_run_id uuid, p_expected_lock_version bigint, p_message_id uuid, p_current_node_id text, p_status text, p_waiting_for text, p_variables jsonb, p_error jsonb, p_outgoing_texts text[], p_outgoing_messages jsonb DEFAULT '[]'::jsonb)
